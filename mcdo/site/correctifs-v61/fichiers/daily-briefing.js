...
// Affiche la vraie date de chaque bloc au lieu d'une seule date qui peut être plus récente que les ventes.
function dashboardHeroDates(d,c,briefing){
  const asOf=d?.asOf||briefing?.date||'',sales=c?.date||'';
  if(sales&&asOf&&sales!==asOf)return 'Bilan du '+briefingDate(asOf)+' · ventes Clearview du '+briefingDate(sales);
  return 'Données au '+briefingDate(sales||asOf);
}
// Signale sur la carte qu'une valeur vient du cache et non d'une lecture récente.
function dashboardCachedTag(block){
  if(!block||(block.dataState!=='cached'&&['ok','none',undefined].includes(block.status)))return '';
  const at=dashboardTimestampLabel(block.lastSuccessfulCollection||block.checkedAt);
  return '<small class="dash-cached-tag">Dernière donnée fiable'+(at?' · '+esc(at):'')+'</small>';
}
function restaurantDashboardHTML(briefing){
  ...
  // v61 : dans les cases du jour, seule la référence du même jour de semaine qu'aujourd'hui est affichée.
  const yearAgoRef=(value,unit='')=>c.serviceDataState==='current'||!dashboardHasNumber(value)||yearAgo?.date!==dashboardYearAgoDate(aujISO())?'':'<small class="dash-year-ref" title="Même jour de semaine l’an passé : '+esc(briefingWeekdayDate(yearAgo.date))+'">'+esc(yearAgoShort)+' : '+esc(briefingNumber(value,Number(value)%1?2:0)+(unit?' '+unit:''))+'</small>';
  const overview='<div class="dash-overview">'+
    '<article class="dash-clickable"'+dashboardInsightAttrs({...satisfaction,title:'Satisfaction 30 jours'})+'><span>Satisfaction 30 jours</span><strong>'+briefingNumber(satisfaction?.value,1)+' %</strong><small>'+dashboardDelta(satisfaction?.delta)+'</small>'+dashboardMiniTrend(m.weekly)+dashboardCachedTag(m)+dashboardInsightBadge(satisfaction||{})+'</article>'+
    '<article class="dash-clickable"'+dashboardInsightAttrs(salesInsight)+'><span>Ventes produit</span><strong>'+briefingMoney(sales.product)+'</strong><small>'+briefingNumber(sales.guestCount)+' transactions · '+briefingMoney(sales.averageCheck)+' moy.</small>'+(c.date&&c.date!==aujISO()?'<small class="dash-stale-tag">'+esc('Journée du '+briefingDate(c.date)+', pas aujourd’hui')+'</small>':'')+dashboardInsightBadge(salesInsight)+'</article>'+
    '<article class="dash-clickable"'+dashboardInsightAttrs(labourInsight)+'><span>Productivité</span>'+(dashboardHasNumber(labour.salesPerLabourHour)?'<strong>'+briefingNumber(labour.salesPerLabourHour,2)+' $</strong><small>VPHT · '+briefingNumber(labour.hours,2)+' h travaillées</small>':'<strong class="dash-unavailable">Indisponible</strong><small>Heures et VPHT du jour non collectés</small>'+yearAgoRef(yearAgoLabour.salesPerLabourHour,'$'))+dashboardInsightBadge(labourInsight)+'</article>'+
